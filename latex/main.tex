...
\begin{itemize}
  ...
  \item \textbf{Engineering feasibility orientation}: Technological assumptions must have precursor foundations in current scientific research (e.g., fusion energy research, quantum computing development, metamaterial research). Program-level references (ITER~\cite{ITER}, IBM quantum hardware~\cite{IBM_Quantum}, SpaceX~\cite{SpaceX_Mars}, NASA Artemis~\cite{NASA_Artemis}, Long Now Foundation~\cite{LongNow_Clock}) are cited as contextual indicators of engineering direction, not as evidential support for specific claims.
...
\textbf{Observation Selection Effects Statement:}

This framework's statistical derivations require an additional epistemological caveat: the Anthropic Principle imposes non-trivial constraints on probabilistic reasoning. As observers, our very existence pre-selects an environment in which life has already emerged---this means we cannot directly infer ``life is common in the universe'' from the fact that ``life exists on Earth'' (Bostrom, 2002~\cite{Bostrom2002}).

Specifically: even if life is extraordinarily rare in the universe ($f_l \to 0$), as long as at least one civilization emerges and produces observers, those observers will necessarily observe that their planet harbors life. This self-selection bias means that an $N=1$ observation cannot constrain the lower bound of $f_l$.

Critically, Sandberg, Drexler \& Ord (2018)~\cite{Sandberg2018} demonstrated that when Drake Equation parameters are treated as realistic probability distributions rather than point estimates, the Fermi Paradox can be dissolved without invoking additional complex mechanisms---the probability of humanity being alone in the observable universe is non-negligible. This result directly constrains the necessity of frameworks that posit hidden civilizations: simpler explanations (we are rare or alone) must be given appropriate prior weight. This framework's conditional structure (``if civilizations exist, then\ldots'') is designed to remain valid regardless of how this prior resolves.

This framework acknowledges this limitation and positions it as supplementary background for Sec.~\ref{sec:abiogenesis} (Abiogenesis Constraint). The framework's core derivations (Parts~II through~VII) employ conditional structure (``if life/civilization already exists, then\ldots''), and therefore are not affected by anthropic bias.
...
\textbf{Evidence Standards:} This framework adopts the general principle articulated in the NASA/NfoLD community life-detection framework~\cite{Green2021}: claims of detection require progression through multiple evidence levels---from scenario generation and observation-path suggestion, through signal identification, non-biological source exclusion, to convergence of independent evidence lines. The sub-models in Parts~III--VII currently occupy the earliest stages (scenario generation and observation-path suggestion); none have yet reached the signal-identification level. This self-assessment should guide interpretation of all claims herein.

\textbf{Epistemological Layering of the Framework:} This hypothesis contains two levels with distinct epistemological statuses:

\begin{enumerate}
  \item \textbf{Falsifiable sub-models} (underground bases, undersea bases, Venus atmospheric habitation, etc.)---each sub-model defines specific weakening or exclusion conditions (see Sec.~\ref{sec:empirical}). The scientific value of these sub-models lies in their capacity to be negated by empirical evidence.
  \item \textbf{Non-falsifiable meta-framework} (distributed non-material civilization forms, see Sec.~\ref{sec:distributed-civ})---this form assumes civilizations leave no physical traces whatsoever, and therefore cannot in principle be excluded by current observational technology. This hypothesis explicitly acknowledges: this meta-framework exceeds the boundaries of falsifiable science; its status is that of a purely speculative thought experiment and should not be evaluated on the same terms as falsifiable sub-models.
\end{enumerate}
...
\subsubsection{The Unresolved Constraint of Abiogenesis Probability}\label{sec:abiogenesis}

The preceding sections have established the following evidence chain: stellar and planetary formation as normality (Sec.~\ref{sec:stellar-formation}) $\to$ widespread habitable zone distribution (Sec.~\ref{sec:earthlike}) $\to$ universal presence of organic molecules (Sec.~\ref{sec:organic-molecules}) $\to$ life can survive in extreme environments (Sec.~\ref{sec:extremophiles}) $\to$ interplanetary rock transfer is physically feasible (Sec.~\ref{sec:panspermia}).

However, a critical break point exists in this evidence chain: \textbf{the transition probability from organic molecules to the first self-replicating system (abiogenesis) is currently completely unknown.} This constitutes the most fundamental source of uncertainty in this framework.

Specifically:

\begin{itemize}
  \item The existence of organic precursors (amino acids, nucleotides, etc.) is a necessary but not sufficient condition for life.
  \item To date, no laboratory experiment has successfully produced a complete self-replicating system spontaneously from an inorganic environment.
  \item Earth is the only known data point for the origin of life ($N=1$), which means we cannot make meaningful statistical inferences about the abiogenesis probability ($f_l$ in the Drake Equation)---its value could range from ${\sim}1$ (near-certain) to $10^{-40}$ (extraordinarily rare). A Bayesian analysis by Lingam et al.\ (2024)~\cite{Lingam2024} provides a quantitative framework for this uncertainty, demonstrating that the posterior distribution of abiogenesis probability per conducive site remains broad and does not yet favor either extreme.
  \item Panspermia (Sec.~\ref{sec:panspermia}) does not resolve this problem; it merely relocates the site of abiogenesis to another celestial body.
\end{itemize}

\textbf{Impact of this constraint on the framework:} If the true value of $f_l$ is extremely low (e.g., $<10^{-20}$), then even with hundreds of billions of habitable planets in the universe, life may still be an extraordinarily rare event. In this scenario, Part~I's conclusion of ``high-probability statistical outcome'' would not hold, but the conditional derivations in Parts~II through~VII (``if life/civilization already exists, then\ldots'') remain valid, as they do not depend on the specific value of $f_l$.
...
\textbf{Physical constraints and speculative status:} The phrase ``only requires materials engineering breakthroughs'' above understates the difficulty. This section (biological--mechanical integration, remote power delivery, consciousness-level transmission) is the most physically unconstrained and speculative part of this framework and should be clearly distinguished from the preceding materials and energy derivations. Two key constraints remain unresolved: (1)~\textbf{the thermodynamic ceiling on remote power delivery}---sustaining metabolic-level continuous power (${\approx}20$~W for the brain, ${\approx}100$~W whole-body) via a directed energy beam incurs severe heating and efficiency losses from absorption and scattering in biological tissue; this is orders of magnitude beyond near-field low-power Qi charging, and no feasible ``wireless life-sustaining power'' pathway currently exists; (2)~\textbf{consciousness-level transmission has no neural-coding basis}---directly encoding concepts, images, and emotions into transmissible neural signals exceeds the current neuroscientific understanding of neural coding and is an unverified speculation. This section should therefore be read as a conditional thought experiment: ``if these unresolved constraints are overcome, then\ldots''.
...
\begin{itemize}
  \item \textbf{Operate in long-term isolation}---If a complete tritium breeding cycle is established, long-term closed operation becomes feasible---deuterium can be extracted almost limitlessly from seawater, and tritium is self-produced via lithium breeding blankets.
...
\textbf{Fuel Cycle Constraint:}

Note that the fuel self-sufficiency claim for D-T fusion requires qualification:

\begin{itemize}
  \item \textbf{Deuterium (D)}: Can be extracted from seawater in virtually unlimited quantities (seawater deuterium content ${\sim}$\SI{33}{mg/L}).
  \item \textbf{Tritium (T)}: Has a half-life of only \SI{12.3}{years} and is virtually non-existent in nature. Reactors must self-produce tritium through a Lithium Breeding Blanket (${}^{6}\text{Li} + n \to T + {}^{4}\text{He}$). The Tritium Breeding Ratio (TBR) must be $>1.0$ to maintain fuel self-sufficiency, imposing rigid requirements on blanket volume and neutronic design---the reactor has a minimum size floor determined by the breeding blanket volume, which constitutes a physical constraint on miniaturization.
\end{itemize}

Alternative fuel pathways (D-D fusion or p-${}^{11}$B fusion) can avoid tritium dependence, but require higher plasma temperatures (D-D: ${\sim}$300--\SI{500}{million~K}; p-${}^{11}$B: ${\sim}$1--\SI{3}{billion~K}) and have lower reaction cross-sections, further escalating the difficulty of materials and confinement engineering. An advanced civilization's choice of fusion technology pathway will directly determine the minimum viable reactor size.
...
\paragraph{Material System Integration Constraints \& Physical Limits.}\label{sec:material-limits}
...
\textbf{Material Compatibility Issues:}

\begin{itemize}
  \item The precise lattice structure required by room-temperature superconductors and the high degree of lattice disorder inherent to high-entropy alloys may produce fundamental conflicts at their interface. If both must operate within the same system (e.g., magnetic confinement coils and structural wall materials of a fusion reactor), interface engineering will become an independent technical bottleneck.
  \item Room-temperature, ambient-pressure superconductivity itself currently has no reproducibly verified experimental realization. Lattice stability limitations within the BCS theory framework mean this technology may require an entirely new superconducting mechanism. If room-temperature superconductivity at ambient pressure is ultimately proven infeasible, the entire derivation chain dependent on it (lossless transmission, fusion magnetic confinement, electromagnetic shielding, stealth) will require alternatives---possible substitutes include high-temperature superconducting materials (liquid nitrogen temperature range, already commercialized) paired with efficient cryogenic systems, but this increases energy consumption and system complexity.
\end{itemize}

\textbf{Physical Limits of Metamaterials:}

\begin{itemize}
  \item \textbf{Bandwidth constraint}: The Kramers-Kronig causality relations impose fundamental limits on the frequency response of metamaterials. Perfect broadband cloaking (simultaneously covering all radar bands, infrared, and visible light) may be physically unrealizable. Practically achievable is narrowband cloaking targeting specific detection technology frequency bands, or adaptive metamaterials that switch between different frequency bands.
  \item \textbf{Scale constraint}: The effective feature scale of metamaterials must be comparable to the target wavelength. Structures effective for radar waves (cm-scale) are ineffective for infrared ($\mu$m-scale); the two require design layers at entirely different scales.
\end{itemize}

\textbf{Inescapable Thermodynamic Constraints:}

\begin{itemize}
  \item Conservation of energy requires that any continuously operating energy system inevitably produces waste heat. Superconducting systems themselves produce no Joule heating, but fusion reactions, biological activity, and computational processes all generate heat. This heat must be discharged to the environment---it can be directionally dissipated or blended with the environmental background, but it cannot be made to disappear.
  \item Therefore, ``stealth'' as used in this framework should be understood as ``\textbf{reducing detectable signatures below the threshold of current or near-future detection technology},'' not ``physically completely invisible.'' As human detection technology advances, this threshold will continue to decrease, and stealth difficulty will continue to increase.
\end{itemize}
...
\midrule
EM Radiation & Metamaterial cloaking & Significantly reduced detectability in specific bands (Kramers-Kronig bandwidth limited) \\
...
Key difference from the Mars model: Venus's runaway greenhouse effect rendered its surface environment (\SI{462}{\degreeCelsius}, \SI{90}{atm})~\cite{NASA_Venus_Fact} completely destructive to all solid-state remnants. However, a habitable zone exists at 50--60~km altitude in Venus's atmosphere where temperature (\SIrange{0}{60}{\degreeCelsius}) and pressure (${\sim}$\SI{1}{atm}) approximate Earth's surface conditions. This provides derivation space for a survival pathway that neither the Earth nor Mars hypotheses can cover---the \textbf{atmospheric habitation model}.
...
\subsection{Bayesian Prior Assessment of Sub-Models}\label{sec:bayesian-priors}
...
\begin{table*}[htbp]
\caption{Bayesian prior assessment of civilization existence sub-models.}
\label{tab:bayesian-priors}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllll}
\toprule
Sub-Model & Prior Plausibility & Supporting Evidence & Primary Falsification Pathway & False-Positive Risk \\
\midrule
Military misjudgment / sensor artifacts & \textbf{High} & Documented sensor failure modes; classified aircraft programs & Exhaustive declassification + sensor calibration studies & Low---well-understood mechanisms \\
Extra-solar observers & \textbf{Low--Moderate} & Exoplanet statistics; Drake-type estimates with realistic uncertainty distributions~\cite{Sandberg2018} & Comprehensive all-sky technosignature survey yielding null result~\cite{Sheikh2025} & Moderate---anthropomorphic projection of observation intent \\
Ancient Mars habitable environment & \textbf{Moderate} & Noachian-era liquid water evidence; atmospheric loss models & Mars sample return showing no biosignatures across multiple sites & Low---habitability $\neq$ biosphere, clearly bounded claim \\
Ancient Mars civilization node & \textbf{Very Low} & None direct; inferred from habitability window & Comprehensive subsurface radar mapping (SHARAD/MARSIS) + sample return & High---large inferential gap from habitability to technosphere \\
Ancient Venus habitable environment & \textbf{Low--Moderate} & GCM models~\cite{Way2016,Way2020}; but constrained by dry interior evidence~\cite{Constantinou2024} & Venus atmospheric entry probe + surface radar (VERITAS/EnVision) & Moderate---model-dependent; recent constraints reduce prior \\
Ancient Venus civilization / aerial biosphere & \textbf{Very Low} & Speculative; phosphine claim unconfirmed~\cite{Snellen2020,Cordiner2022}; metabolic models challenged~\cite{Bains2021} & In-situ atmospheric chemistry analysis; cloud-deck sampling & High---multiple unfavorable constraints since 2020 \\
Ancient Earth underground civilization & \textbf{Very Low} & Silurian hypothesis framework~\cite{Schmidt2019}; no direct evidence & Global subsurface imaging + deep-sea floor survey & Moderate---geological record destruction limits testability \\
Non-biological intelligence probes & \textbf{Low} & Physical feasibility of self-replicating probes~\cite{Freitas1980} & Interstellar object spectral/compositional analysis & Moderate---natural object misidentification \\
Directed migration (Mars$\to$Venus$\to$Earth relay) & \textbf{Very Low} & Temporal ordering of habitability windows; panspermia feasibility & Multi-body biosignature comparison; genetic analysis of any extraterrestrial biology found & High---chain of low-prior assumptions multiplied together \\
Distributed non-material civilization & \textbf{Not assessable} & None; unfalsifiable by design & No empirical pathway exists & N/A---outside scope of falsifiable science \\
\bottomrule
\end{tabular}
}
\end{table*}

\textbf{Reading note:} ``Very Low'' prior does not mean ``impossible''---it means the claim requires substantially more evidence than currently available before it can compete with simpler explanations. In particular, the directed-migration relay (Mars $\to$ Venus $\to$ Earth) is a product of several individually ``Very Low'' priors, so its compound posterior is the lowest of any branch---it should be read as a logical possibility retained for structural completeness and falsifiability, not as a credible scenario.
...
Advanced civilizations may consciously choose not to intervene in Earth's civilizational development (similar to the ``Zoo Hypothesis''~\cite{Ball1973}). Possible reasons include:
...
\textbf{Parameter Sensitivity Warning:} The above habitability window estimate is highly dependent on the initial condition assumptions of the climate model. Key sensitive parameters include:

\begin{table*}[htbp]
\caption{Parameter sensitivity of the Venus habitability model.}
\label{tab:venus-sensitivity}
\begin{tabular}{lll}
\toprule
Parameter & Baseline Assumption & If Assumption Changes \\
\midrule
Early rotation rate & Slow (similar to present) & Faster rotation $\to$ altered circulation, shorter window \\
Initial water inventory & Earth-like & Less water $\to$ oceans may never have formed \\
Carbon cycle efficiency & Effective carbonate-silicate & Low efficiency $\to$ earlier runaway greenhouse \\
Early solar luminosity & 70--85\% of present & Relatively stable constraint, lower uncertainty \\
\bottomrule
\end{tabular}
\end{table*}
...
\textbf{Post-2024 Observational Constraints:} Since the original Venus habitability models were published, several lines of evidence have tightened the constraints on Venus's habitable past:

\begin{itemize}
  \item \textbf{Dry interior constraint}---Modeling of Venus's interior evolution indicates that Venus's mantle is significantly drier than Earth's, consistent with a long-term arid surface history. This reduces the prior probability that Venus sustained liquid water oceans for billions of years~\cite{Constantinou2024}.
  \item \textbf{Phosphine re-analysis}---Independent re-analysis of the original ALMA data found no statistically significant phosphine detection~\cite{Snellen2020}. SOFIA observations provided a strict upper limit well below the originally claimed abundance~\cite{Cordiner2022}.
  \item \textbf{Metabolic model challenges}---Proposed biochemical metabolic pathways for a Venus cloud-layer biosphere have been shown to be inconsistent with observed atmospheric chemistry---the predicted chemical signatures do not match what is actually measured~\cite{Bains2021}.
  \item \textbf{Amino acid stability}---Concentrated sulfuric acid in Venus's cloud layer rapidly destroys amino acid backbones, posing a fundamental challenge to Earth-like biochemistry in the Venus atmosphere~\cite{Petkowski2024}.
  \item \textbf{Dipeptide stability}---Follow-up studies on peptide bond stability under Venus cloud conditions further constrain the viability of complex organic chemistry in this environment~\cite{Seager2024dipeptide}.
\end{itemize}

\textbf{Balanced assessment:} The Venus habitability hypothesis now occupies a more constrained position than in the pre-2024 literature. The early habitability window remains physically possible under specific parameter combinations, but the overall prior has been reduced. This framework retains Venus as a bounded, low-prior branch rather than a co-equal pathway alongside Mars.

As with the Mars chapter (Sec.~\ref{sec:mars-habit}), early Venus life may have originated through two pathways: independent local abiogenesis, or natural transfer from other bodies via the Panspermia mechanism (see Sec.~\ref{sec:panspermia}). Notably, because Venus's orbital distance to Earth is shorter (approximately \SI{0.25}{AU} at closest approach), the efficiency of interplanetary meteoritic transfer may be higher than that of the Mars--Earth pathway, giving panspermia a higher prior probability in the Venus scenario.
...
\begin{itemize}
  ...
  \item \textbf{Complete surface destruction}---The current Venus surface temperature of \SI{462}{\degreeCelsius} and pressure of \SI{90}{atm}~\cite{NASA_Venus_Fact} would completely dissolve any solid-state civilization remnants.
\end{itemize}
...
Gravity & \SI{8.87}{m/s^2}~\cite{NASA_Venus_Fact} & 90\% of Earth \\
...
\begin{itemize}
  \item \textbf{Buoyant structure engineering}---Exploiting the density difference between Venus's atmospheric composition (primarily CO$_2$) and breathable gases (N$_2$/O$_2$); a nitrogen-oxygen gas mixture is naturally buoyant in Venus's atmosphere. If hydrogen (H$_2$) or helium (He) is used as the lifting gas instead, buoyancy can increase by approximately 1.5--2$\times$, but hydrogen introduces explosion risk (in oxygen-containing gas mixtures), while helium faces resource acquisition constraints. Practical designs must trade off between buoyancy gain and safety.
...
\textbf{Open Questions on Energy Feasibility:}

The atmospheric habitation model faces more stringent energy constraints than underground or undersea models:

\begin{table*}[htbp]
\caption{Energy feasibility analysis for Venus atmospheric habitation.}
\label{tab:venus-energy}
\begin{tabular}{lll}
\toprule
Energy Option & Theoretical Feasibility & Engineering Constraints \\
\midrule
Atmospheric chemical gradient & Low (${\sim}10^{0}$--$10^{1}$~W/m$^2$) & Insufficient for civilization-level activity \\
Solar (above cloud layer) & Medium (${\sim}$\SI{2600}{W/m^2}) & Sulfuric acid cloud attenuation \\
Nuclear fusion & Highest density & Reactor mass vs.\ buoyancy budget conflict \\
Wind (super-rotation) & Medium (${\sim}$\SI{100}{m/s}) & Requires large structures \\
\bottomrule
\end{tabular}
\end{table*}

This energy dilemma is a key unresolved constraint. If future materials science cannot reduce fusion reactor mass to a range bearable by buoyant structures, the feasibility of atmospheric habitation will depend on whether low-energy-density combinations (chemical gradient + solar + wind) suffice to sustain civilization activity---an open question requiring quantitative analysis.
...
\begin{itemize}
  ...
  \item This further strengthens the Appendix~A derivation of ``multi-source model ensembles''---``alien'' phenomena on Earth may contain remnants of different planetary origins.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Surface remnants are completely inaccessible}---The \SI{462}{\degreeCelsius}, \SI{90}{atm}~\cite{NASA_Venus_Fact} environment limits any lander's lifespan to extremely short durations (the Soviet Venera series survived a maximum of approximately 127 minutes)~\cite{NASA_Venera13}.
  \item \textbf{Geological timescale masking is more thorough}---The Venus surface underwent a global volcanic resurfacing event approximately 300--500 million years ago~\cite{Strom1994}, erasing all earlier geological records.
...
\begin{itemize}
  \item \textbf{Extreme exploration difficulty in high-pressure deep-sea zones}---Human direct visual observation coverage of the deep seafloor is approximately 0.001\% (2025 \textit{Science Advances} study)~\cite{Bell2025}, with exploration of the hadal zone ($>$\SI{6000}{m}) even more sparse.
...
% PART VII
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Part~VI --- Cross-Galactic Mission Architecture \& Human Positioning}\label{sec:part7}
...
\textbf{Distinction Criteria for the Two Tracks:}
...
\begin{table*}[htbp]
\caption{Distinction criteria for natural panspermia vs.\ directed migration.}
\label{tab:distinction-criteria}
\begin{tabular}{lll}
\toprule
Criterion & Natural Panspermia Expected & Directed Migration Expected \\
\midrule
Genetic code & Shared biochemical systems (if homologous) & Possible artificial gene editing traces \\
Isotope ratios & Natural nucleosynthesis patterns & Possible non-natural ratios \\
Biochemical complexity & Evolutionary gradient & Sudden high-complexity appearance \\
Spatial distribution & Random / impact debris trajectories & Non-random, concentrated at habitable sites \\
Accompanying materials & Natural rock matrix & Possible non-natural materials \\
\bottomrule
\end{tabular}
\end{table*}
...
\midrule
Gamma-ray burst (GRB) & High-energy radiation released by nearby stellar collapse & Thousands of light-years & Possible link to the Ordovician mass extinction~\cite{Melott2004} \\
...
Supernova explosion & Nearby massive star detonation & Tens of light-years & Pliocene seabed iron-60 deposits~\cite{Wallner2016} \\
\bottomrule
\end{tabular}
}
\end{table*}
...
\textbf{Competing Hypotheses Statement:} The structural similarity of global myths admits multiple non-mutually-exclusive explanatory models, and this framework does not reject any of them:

\begin{enumerate}
  \item \textbf{Cognitive universality hypothesis} (modern extension of Jungian archetype theory)---the shared neural architecture of the human brain naturally produces similar narrative structures.
  \item \textbf{Common natural experience hypothesis}---global natural disasters such as floods, volcanoes, and earthquakes produce parallel narratives in independent cultures.
  \item \textbf{Cultural diffusion hypothesis}---cultural contact along early human migration routes propagated core narrative motifs.
  \item \textbf{Civilization severance encoding hypothesis} (proposed by this framework)---structural similarities reflect compressed memories of the same one or more civilization severance events.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{Rapid stepwise expansion of cognitive capacity}: Human brain volume approximately tripled over ${\sim}$2 million years~\cite{Rightmire2004} (\textit{Homo habilis} $\to$ \textit{Homo sapiens}). Mainstream evolutionary explanations (social intelligence hypothesis, language co-evolution, cultural ratchet effect, etc.) already provide multi-factor models. However, within the cross-galactic deployment framework, an additional non-mutually-exclusive speculation is: if Earth received external biological seeding, the introduced genetic material may have already contained gene combinations with high cognitive potential, making the substrate of natural selection more favorable for rapid cognitive expansion. This speculation does not replace evolutionary explanations but is superimposed upon them.
\end{itemize}
...
% PART VIII
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Part~VII --- Verifiable Observation Pathways}\label{sec:part8}
...
\subsubsection{Instrument Resolution \& Detection Thresholds}\label{sec:instrument-thresholds}
...
\begin{table*}[htbp]
\caption{Instrument resolution and detection thresholds for each observation pathway.}
\label{tab:instrument-thresholds}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lllll}
\toprule
...
\midrule
Satellite gravimetry (GRACE-FO~\cite{NASA_GRACEFO}) & Underground base (mass anomaly) & ${\sim}10^{10}$~kg localized mass concentration & Effective spatial resolution ${\sim}$300~km; sensitive to large-scale mass redistribution & \textbf{Weak} for structures $<$100~km; strong for continent-scale anomalies \\
Muon tomography~\cite{Alvarez1970} & Underground cavity & Density contrast $>$5\% over ${\sim}$100~m scale & Penetration depth ${\sim}$1~km in rock; ${\sim}$10~m resolution at shallow depths & \textbf{Strong} for shallow ($<$1~km), localized targets; negligible for deep or distributed structures \\
Seabed multibeam sonar (Seabed 2030~\cite{Seabed2030}) & Undersea base & Geometric anomaly $>$100~m on seafloor & 27.3\% of ocean floor mapped to modern standards (as of 2025); resolution ${\sim}$100~m at abyssal depths & \textbf{Weak} globally (73\% unmapped); strong in surveyed regions \\
Deep-sea visual observation~\cite{Bell2025} & Undersea artificial structures & Visual-scale artificial features & $<$0.001\% of deep seafloor directly observed & \textbf{Very weak}---near-zero global coverage \\
Mars subsurface radar (SHARAD~\cite{NASA_SHARAD} / MARSIS~\cite{ESA_MARSIS}) & Mars subsurface structures & Dielectric contrast at $>$10~m scale & SHARAD: ${\sim}$15~m vertical resolution, ${\sim}$1~km penetration; MARSIS: ${\sim}$150~m vertical, ${\sim}$5~km penetration & \textbf{Moderate} for large cavities; weak for small or deep structures \\
Venus SAR (VERITAS~\cite{NASA_VERITAS} / EnVision~\cite{ESA_EnVision}) & Venus subsurface anomalies & Surface/near-surface geometric anomaly & VERITAS: ${\sim}$30~m resolution (status uncertain); EnVision VenSAR: ${\sim}$30~m (launch ${\sim}$2031) & \textbf{Pending}---instruments not yet operational \\
Technosignature survey~\cite{Sheikh2025} & Extra-solar observers, directed EM & Earth-level radio/optical leakage at interstellar distances & Detectable across 13 orders of magnitude depending on mode; strongest radio modes detectable to ${\sim}$100~pc & \textbf{Moderate} for nearby sources; weak for distant or deliberately quiet civilizations \\
Atmospheric chemistry probes & Venus aerial biosphere & Non-equilibrium chemical species (ppb-level) & No long-duration Venus atmospheric probe currently operational & \textbf{Pending}---no in-situ data available \\
\bottomrule
\end{tabular}
}
\end{table*}

\textbf{Key implication:} The claim ``if we find nothing, the sub-model is weakened'' must be qualified by instrument capability. For sub-models where current instruments lack the resolution to detect the predicted signature, a null result is uninformative rather than constraining. This table should be updated as new missions become operational.
...
\subsubsection{Interstellar Object Observation}\label{sec:interstellar-objects}

\begin{itemize}
  \item \textbf{Interstellar object spectral and orbital analysis}---Conducting spectral, orbital, and non-gravitational acceleration analysis of interstellar objects entering the solar system (e.g., 1I/'Oumuamua~\cite{Meech2017}, 2I/Borisov~\cite{Guzik2020}, 3I/ATLAS~\cite{NASA_3IATLAS}), searching for non-natural signatures.
  \item \textbf{Vera Rubin Observatory}~\cite{Ivezic2019}---The Rubin Observatory has begun scientific operations, issuing its first batch of scientific alerts on 2026-02-25~\cite{RubinObs2026}. Its Legacy Survey of Space and Time (LSST) is expected to detect dozens of interstellar objects per year, dramatically expanding the sample size and enabling statistical characterization of the interstellar object population.
  \item \textbf{Interstellar object intercept missions}---Rapid intercept mission concepts such as ESA Comet Interceptor~\cite{Snodgrass2019} could enable close-range observation or even sampling of interstellar objects.
\end{itemize}
...
\textbf{Determination criterion}: If large samples of interstellar objects all exhibit spectral and orbital characteristics consistent with natural bodies, then the prior probability of the Von Neumann probe model (Sec.~\ref{sec:non-bio}) should be adjusted downward. Conversely, if any interstellar object exhibits features unexplainable by natural processes (anomalous albedo, non-gravitational acceleration without cometary activity, anomalous shape, etc.), further investigation is warranted.
...
\textbf{Determination criterion}: If Venus atmospheric chemical analysis reveals no non-natural compounds and long-term upper atmosphere monitoring detects no artificial structural features, then the Venus atmospheric habitation model can be weakened. If Venus surface radar penetration finds no artificial structural traces beneath the global resurfacing layer~\cite{Strom1994}, then the ancient Venus surface civilization model can be further weakened.
...
\section{Cognitive Deconstruction of the ``Alien'' Concept}\label{sec:appendix-alien}

\textit{Note: This appendix presents a cultural-analogy framework for the semantic decomposition of the term ``alien.'' It is positioned as supplementary context rather than part of the core evidential chain. Part~III (Sec.~\ref{sec:part3}) provides the structural analysis; this appendix explores the cognitive and linguistic dimensions.}

\subsection{Limitations of the Single-Species Assumption}\label{sec:single-species}
...
\subsection{Multi-Source Model Ensemble}\label{sec:multi-source}
...
\subsection{Civilization Morphological Diversity}\label{sec:morphological}
...
\subsection{Human Cognitive Projection}\label{sec:cognitive-projection}
...
\textbf{Data and Code Availability:} The complete source files, Markdown versions (English and zh-TW / 正體中文), and independent verification report are available at \url{https://github.com/djchrisssssss/cosmic-diaspora-hypothesis}.

\textbf{License:} This work is licensed under the Creative Commons Attribution 4.0 International License (CC BY 4.0). To view a copy, visit \url{https://creativecommons.org/licenses/by/4.0/}.
